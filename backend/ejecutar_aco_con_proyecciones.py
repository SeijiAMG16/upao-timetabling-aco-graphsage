...
from aco_simple import aco_simple_sin_restricciones
...
def aplicar_local_search_tpl(solucion_inicial, max_intentos=200):
    """
    Búsqueda local simple para mejorar T→P→L intercambiando slots
    """
    dias_num = {'LUNES': 1, 'MARTES': 2, 'MIÉRCOLES': 3, 'JUEVES': 4, 'VIERNES': 5, 'SÁBADO': 6}
    
    def get_timestamp(asig):
        """Convierte asignación a timestamp comparable"""
        dia = asig['day']
    ...
    # Contar violaciones actuales
    def contar_violaciones(sol):
        """Cuenta violaciones T→P→L en la solución"""
        from collections import defaultdict
        
        asigs_por_curso = defaultdict(list)
        for asig in sol:
            asigs_por_curso[asig['course_id']].append(asig)
        
        violaciones = 0
        for course_id, asigs in asigs_por_curso.items():
            ...
            if teorias and practicas:
                max_t_ts = max(get_timestamp(t) for t in teorias)
                min_p_ts = min(get_timestamp(p) for p in practicas)
                if max_t_ts >= min_p_ts:
                    violaciones += 1
            
            # P→L
            if practicas and labs:
                max_p_ts = max(get_timestamp(p) for p in practicas)
                min_l_ts = min(get_timestamp(l) for l in labs)
                if max_p_ts >= min_l_ts:
                    violaciones += 1
            ...
            if teorias and labs:
                max_t_ts = max(get_timestamp(t) for t in teorias)
                min_l_ts = min(get_timestamp(l) for l in labs)
                if max_t_ts >= min_l_ts:
                    violaciones += 1
        
        return violaciones
    
    violaciones_iniciales = contar_violaciones(solucion_inicial)
    print(f"📊 Violaciones T→P→L iniciales: {violaciones_iniciales}")
    
    mejor_solucion = [asig.copy() for asig in solucion_inicial]
    mejoras_realizadas = 0
    
    for intento in range(max_intentos):
        # Seleccionar dos asignaciones aleatorias del mismo tipo (para no romper mucho)
        idx1, idx2 = random.sample(range(len(mejor_solucion)), 2)
        
        # Intentar swap
        temp_sol = [asig.copy() for asig in mejor_solucion]
        
        # Intercambiar slots (día, hora, aula)
        temp_sol[idx1]['day'] = mejor_solucion[idx2]['day']
        temp_sol[idx1]['start_time'] = mejor_solucion[idx2]['start_time']
        temp_sol[idx1]['end_time'] = mejor_solucion[idx2]['end_time']
        temp_sol[idx1]['classroom_id'] = mejor_solucion[idx2]['classroom_id']
        
        temp_sol[idx2]['day'] = mejor_solucion[idx1]['day']
        temp_sol[idx2]['start_time'] = mejor_solucion[idx1]['start_time']
        temp_sol[idx2]['end_time'] = mejor_solucion[idx1]['end_time']
        temp_sol[idx2]['classroom_id'] = mejor_solucion[idx1]['classroom_id']
        
        # Verificar si mejora
        violaciones_nuevas = contar_violaciones(temp_sol)
        
        if violaciones_nuevas < contar_violaciones(mejor_solucion):
            mejor_solucion = temp_sol
            mejoras_realizadas += 1
            
            if mejoras_realizadas % 10 == 0:
                print(f"   ✅ Mejoras: {mejoras_realizadas} | Violaciones: {violaciones_nuevas}")
    
    violaciones_finales = contar_violaciones(mejor_solucion)
    mejora = violaciones_iniciales - violaciones_finales
    
    print(f"📊 Violaciones T→P→L finales: {violaciones_finales}")
    print(f"📈 Mejora total: -{mejora} violaciones")
    print(f"🔄 Swaps exitosos: {mejoras_realizadas}/{max_intentos}")
    print("="*80)
...
def main():
    ...
    # PASO 4: Asignar horarios con ACO SIMPLE (sin restricciones)
    solucion_inicial, hist_asig, hist_conf = aco_simple_sin_restricciones(
        secciones,
        datos['profesores'],
        datos['aulas_por_tipo'],
        datos['slots_tiempo'],
        num_hormigas=20,  # Aumentado para mejor exploración
        max_iteraciones=15  # Aumentado para converger mejor
    )
    
    # PASO 4.5: LOCAL SEARCH para mejorar T→P→L
    print("\n" + "="*80)
    print("🔍 APLICANDO LOCAL SEARCH PARA MEJORAR T→P→L")
    print("="*80)
    solucion = aplicar_local_search_tpl(solucion_inicial, max_intentos=200)
