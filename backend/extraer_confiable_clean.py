...
DIAS_SEMANA = ['LUNES', 'MARTES', 'MIRCOLES', 'JUEVES', 'VIERNES', 'SBADO']
...
def normalizar_nombre_curso(nombre):
    ...
    # Remover texto entre parntesis
    nombre = re.sub(r'\([^)]*\)', '', nombre)
    
    # Limpiar espacios y convertir a maysculas
    nombre = nombre.strip().upper()
    
    # Remover caracteres especiales y nmeros de seccin
    nombre = re.sub(r'\d{4,}', '', nombre)  # Remover cdigos de 4+ dgitos
...
def encontrar_curso_por_nombre(nombre_curso, cursos):
    ...
    if mejores_matches:
        # Ordenar por nmero de coincidencias
...
def extraer_nombre_profesor_de_celda(contenido):
    ...
    # Buscar patrn "- NombreProfesor" al final
    match = re.search(r'-\s*([A-Za-z\s]+?)(?:\s|$)', contenido)
    if match:
...
def es_celda_bloqueada(cell):
    """Verifica si una celda est bloqueada (color rojo EXACTO)"""
...
def extraer_asignaciones_y_restricciones_v2():
    """Extraccin confiable de asignaciones y restricciones"""
    ...
    nombres_encontrados = defaultdict(set)  # Para estadsticas
    ...
    for sheet_name in wb.sheetnames:
        ...
        if not fila_encabezado:
            print("    No se encontr fila de encabezados")
            continue
        
        # Mapear columnas a das
        ...
        print(f"   Das encontrados: {list(columnas_dias.values())}")
        ...
        for row_idx in range(fila_inicio, min(fila_inicio + 20, ws.max_row + 1)):
            ...
            for col_idx, dia in columnas_dias.items():
                ...
                # Verificar si est bloqueada (ROJO)
                if es_celda_bloqueada(cell):
                    restricciones_hoja[dia][bloque_idx] = True
                    print(f"   Restriccin: {dia} bloque {bloque_idx+1} ({BLOQUES_TIEMPO[bloque_idx][0]})")
                
                # Verificar si hay asignacin
                if cell.value and isinstance(cell.value, str) and len(cell.value.strip()) > 5:
                    ...
                    if nombre_prof:
                        ...
                        # Si an no hemos identificado el profesor de esta hoja
        ...
        # Guardar restricciones si se identific el profesor
        if profesor_id_hoja:
...
def insertar_en_bd(asignaciones, restricciones):
    ...
    print("  Restricciones anteriores eliminadas")
    ...
    # 2. Limpiar e insertar asignaciones histricas
    print("\n" + "="*80)
    print(" INSERTANDO ASIGNACIONES HISTRICAS EN BD")
    print("="*80)
    ...
    print(f" {total_asig} asignaciones histricas insertadas")
